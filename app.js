...
function speak(text,rate=.88){
  ...
  const voice=voices.find(v=>v.lang==='en-US'&&/Samantha|Ava|Siri/i.test(v.name))
    ...
    ||null;
  // Speak one sentence at a time. Queuing a whole paragraph at once can be
  // silently dropped by iPad Safari.
  const run=++speechRun;
  activeUtterances=[];
  if(synth.speaking||synth.pending)synth.cancel();
  if(synth.paused)synth.resume();
  const parts=(phrase.match(/[^.!?]+[.!?]+|[^.!?]+$/g)||[phrase]).map(s=>s.trim()).filter(Boolean);
  const play=index=>{
...
function openLesson(id){state.current=id;state.editing=false;state.selectedHotspot=null;const l=lessons.find(x=>x.id===id);if(!l)return;$('#modalTitle').textContent=`Lesson ${l.id}｜${l.title}`;$('#modalSub').textContent=l.titleZh;renderImagePane(l);const c=l.content;$('#wordsPane').innerHTML=c&&c.vocabulary?`<div class="words">${c.vocabulary.map((v,i)=>`<button class="word" data-word-index="${i}"><strong>${esc(v[0])}</strong><span>${esc(v[1])}</span></button>`).join('')}</div>`:`<div class="empty">本課單字資料尚未匯入。</div>`;$('#wordsPane').querySelectorAll('[data-word-index]').forEach(btn=>btn.onclick=()=>speak(c.vocabulary[Number(btn.dataset.wordIndex)][0]));$('#readingPane').innerHTML=c?`<div class="reading"><p>${esc(c.reading||'')}</p><button id="readAllBtn" class="primary read-all">🔊 全文朗讀</button></div>`:`<div class="empty">本課朗讀資料尚未匯入。</div>`;const readAllBtn=$('#readAllBtn');if(readAllBtn)readAllBtn.onclick=()=>speak(c.reading||'',.65);$('#modal').classList.add('open');showTab('image');updateDoneBtn();}
